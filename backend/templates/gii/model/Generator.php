...
namespace backend\templates\gii\model;
...
class Generator extends \yii\gii\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'nsDb', 'tableName', 'modelClass', 'baseClass'], 'filter', 'filter' => 'trim'],
            [['nsDb'], 'filter', 'filter' => function($value) { return trim($value, '\\'); }],

            [['db', 'nsDb', 'tableName', 'baseClass'], 'required'],
            [['db', 'modelClass'], 'match', 'pattern' => '/^\w+$/', 'message' => 'Only word characters are allowed.'],
            [['nsDb', 'baseClass'], 'match', 'pattern' => '/^[\w\\\\]+$/', 'message' => 'Only word characters and backslashes are allowed.'],
            [['tableName'], 'match', 'pattern' => '/^(\w+\.)?([\w\*]+)$/', 'message' => 'Only word characters, and optionally an asterisk and/or a dot are allowed.'],
            [['db'], 'validateDb'],
            [['nsDb'], 'validateNamespace'],
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            'nsDb' => 'This is the namespace of the ActiveRecord class to be generated, e.g., <code>app\models</code>',
    ...
    {
        return ['model.php','modelDB.php','modelBase.php'];
    }
    ...
    {
        return array_merge(parent::stickyAttributes(), ['nsDb', 'db', 'baseClass', 'generateRelations', 'generateLabelsFromComments']);
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $files[0] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->nsDb)) . '/' . $className . 'DB.php',
                $this->render('modelDB.php', $params)
            );
            $params['className'] = $className.'Base';
            $files[1] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->nsBase)) . '/' . $className . 'Base.php',
                $this->render('modelBase.php', $params)
            );

            $apps = Yii::$app->params['apps'];

            $i = 2;
            foreach($apps as $app){
                $params['className'] = $className;
                $path = $params['ns'] = $app.'\models';
                $files[$i] = new CodeFile(
                    Yii::getAlias('@' . str_replace('\\', '/', $path)) . '/' . $className .'.php',
        ...
        foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
            ...
            foreach ($table->foreignKeys as $refs) {
                ...
                $relations[$className][$relationName] = [
                    "return \$this->hasOne(".$refClassName."DB::className(), $link);",
                    $refClassName,
                    false,
                ];
                ...
                $relations[$refClassName][$relationName] = [
                    "return \$this->" . ($hasMany ? 'hasMany' : 'hasOne') . "(".$className."DB::className(), $link);",
                    $className,
                    $hasMany,
                ];
            }
            ...
            $relations[$className0][$relationName] = [
                "return \$this->hasMany(".$className1."DB::className(), $link)->viaTable('" . $this->generateTableName($table->name) . "', $viaLink);",
                $className1,
                true,
            ];
            ...
            $relations[$className1][$relationName] = [
                "return \$this->hasMany(".$className0."DB::className(), $link)->viaTable('" . $this->generateTableName($table->name) . "', $viaLink);",
                $className0,
                true,
            ];
        }
    ...
    {
        $this->nsDb = ltrim($this->nsDb, '\\');
        $path = Yii::getAlias('@' . str_replace('\\', '/', $this->nsDb), false);
        if ($path === false) {
            $this->addError('nsDb', 'Namespace must be associated with an existing directory.');
        }
    }
